...
public class UploadLogoTask<P, S, T> extends AsyncTask<Void, Void, T> {
    ...
    public UploadLogoTask(Context context, ArrayList<BasicNameValuePartner> dataArr, Bitmap bitmap, String functioName, CallBacks callBacks) {
...
//        dialog = Tool.createLoadingDialog(context);
//        dialog.show();
        ...
        try {
            ...
            if (bitmap != null){
                ds.writeBytes("Content-Disposition: form-data; " + "name=\"logo\";filename=\"" + newName + "\"" + ";boundary=" + boundary);
...
//        }
        ...
        for (int i = 0; i < params.size(); i++) {
            if (bitmap == null && i == 0){
                sb += (twoHyphens + boundary + end + "Content-Disposition: form-data; name=\"" + params.get(i).getKey() + "\"" + ";boundary=" + boundary);
            }
