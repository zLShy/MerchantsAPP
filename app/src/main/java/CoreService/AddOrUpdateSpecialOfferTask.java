...
public class AddOrUpdateSpecialOfferTask<P, S, T> extends AsyncTask<Void, Void, T> {
    ...
    public AddOrUpdateSpecialOfferTask(Context context, ArrayList<BasicNameValuePartner> dataArr, ArrayList<Bitmap> bitmaps, String functioName, CallBacks callBacks) {
...
//            ds.writeBytes("Content-Disposition: form-data; " + "name=\"coupon_icon\";filename=\"" + newName + "\"" + ";boundary=" + boundary);
//            ds.writeBytes(end + end);
//
//            ByteArrayOutputStream baos = new ByteArrayOutputStream();
//            headerBitmap.compress(Bitmap.CompressFormat.PNG, 100, baos);
...
//        }
        ...
        try {
            for (int n = 0; n < bitmaps.size(); n++){
                output.writeBytes(twoHyphens + boundary + end + "Content-Disposition: form-data; name=\"" + "path" + Integer.toString(fileIndex) + "\";filename=\""  + getPhotoFileName() + "\"");
                output.writeBytes(lineEnd + end);
                fileIndex++;


                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                bitmaps.get(n).compress(Bitmap.CompressFormat.PNG, 100, baos);
                InputStream fStream = new ByteArrayInputStream(baos.toByteArray());
